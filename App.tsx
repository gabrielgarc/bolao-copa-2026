...
import { GROUP_DEFINITIONS, SIMULATED_TODAY } from './constants';
...
const App: React.FC = () => {
  const [currentView, setCurrentView] = useState<AppView>(AppView.MATCHES);
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      setIsLoading(true);
      try {
        const [matchesData, rankingsData, predsData, userData] = await Promise.all([
          MatchService.getAll(),
  ...
  const groupStats = useMemo(() => {
    if (currentStage !== 'GROUPS' || allMatches.length === 0) return [];

    const teams = GROUP_DEFINITIONS[activeGroup] || [];
  ...
  }, [activeGroup, currentStage, predictions, allMatches, currentView]);

  const todayStr = SIMULATED_TODAY;
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                <div className="w-12 h-12 border-4 border-yellow-400 border-t-transparent animate-spin mb-4"></div>
                <p className="text-yellow-400 font-bold animate-pulse uppercase">Carregando dados...</p>
            </div>
        );
    }
  ...
  return (
    <div className={`min-h-screen pb-20 transition-colors duration-500 ${currentView === AppView.OFFICIAL_RESULTS ? 'bg-red-900' : 'bg-green-800'} bg-[url('https://www.transparenttextures.com/patterns/pixel-weave.png')]`}>
      ...
      <main className="p-3 md:p-8">
        ...
        {currentView === AppView.OFFICIAL_RESULTS && renderMatchesView(true)}
        ...
        {currentView === AppView.LEADERBOARD && (
            <div className="max-w-2xl mx-auto">
                <PixelCard className="bg-yellow-100">
                    ...
                    {isLoading ? (
                         <p className="text-center py-10 animate-pulse text-gray-700 font-bold uppercase">Buscando Ranking...</p>
                    ) : (
                        <div className="space-y-4">
                            {[...leaderboard]
